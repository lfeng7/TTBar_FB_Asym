...
parser.add_option('--ismcatnlo', metavar='F', type='string', action='store', default='no', dest='ismcatnlo', help='') ## Sets whether we have to use the alternate algorithm
																													  ## for the mcatnlo GenParticles structure
...
for event in events :
	...
	if count!=0 and count%50000 == 0 :
		...
		print '----------------		 weights 	 -----------------'
		print '---positive weights: '+str(count_positive_weights)+' (%.4f%%)'%(100.0*count_positive_weights/count)
...
#			count_positive_weights+=1
	...
	if GenHandle.isValid() :
		...
		for ig in GenParticles :
			...
			if nMothers != 0 :
				s = s + '-> '
			s = s + '(' + getId(ig.pdgId()) +'('+str(ig.status())+')'+') '
			if nDaughters != 0 :
				s = s + '-> { '
			for i in range(nDaughters) :
				s = s + getId(ig.daughter(i).pdgId()) + '('+str(ig.daughter(i).status())+') '
			if nDaughters != 0 :
				s = s + '}'
