...
parser.add_option('--top_type',   type='int', 	 action='store', default=1, 	  dest='top_type',	  help='Specify top type "1" (fully merged) or "2" (partially merged), default is "1"')
parser.add_option('--max_events', type='int',    action='store', default=-1,	  dest='max_events',  help='Maximum number of events to process (default is -1 for "all")')
parser.add_option('--print_every',type='int',    action='store', default=1000,	  dest='print_every', help='Print progress after how many events?')
parser.add_option('--n_jobs', 	  type='int',    action='store', default=1,		  dest='n_jobs',	  help='Number of total grid jobs')
parser.add_option('--i_job', 	  type='int',    action='store', default=0,		  dest='i_job',	   	  help='Which job is this in the sequence?')
#Sample options
parser.add_option('--name', 		 type='string', action='store', 			  	dest='name', 		    help='Name of sample or process (used to name output files, etc.)')
parser.add_option('--data', 		 type='string', action='store', default='no', 	dest='data', 		    help='Set to "yes" if file is a data file (MC file assumed)')
parser.add_option('--generator', 	 type='string', action='store', default='none', dest='generator', 		help='Monte Carlo generator for this file (powheg, madgraph, pythia8); default is "none"')
parser.add_option('--cross_section', type='float',  action='store', default=1.0,  	dest='cross_section', 	help='Cross section of process')
...
filename+='_tree.root'
#Initialize analyzer
analyzer = treemaker(filename, options.data, options.generator, options.event_type, options.sideband, options.leptons, options.top_type, options.cross_section/options.n_events)
