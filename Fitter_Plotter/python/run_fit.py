...
parser.add_option('--data_input_file',type='string',action='store',default='data_input',dest='data_input_file',help='Path to input TTree file made from data to fit to')
...
if options.on_grid=='yes' :
	...
	input_file_path+='.'
MC_templates_filename = input_file_path+options.templates_file
data_TTree_filename   = input_file_path+options.data_input_file
if not MC_templates_filename.endswith('.root') :
	MC_templates_filename+='.root'
if not data_TTree_filename.endswith('.txt') :
	data_TTree_filename+='.txt'
...
#make a new fitter
fit_obj = fitter(runname,options.on_grid,MC_templates_filename,data_TTree_filename,output_name)
